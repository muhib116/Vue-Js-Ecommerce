...
@section('content')
                ...
                <div class="row page-titles">
                    ...
                    <div class="col-md-7 align-self-center text-right">
                        <div class="d-flex justify-content-end align-items-center">
                           ...
                           <a class="btn btn-info d-none d-lg-block m-l-15" href="{{route('vendor.dashboard')}}"><i class="fa fa-plus-circle"></i> Enlist To Shop</a>
                        </div>
                    </div>
                </div>
                <!-- ============================================================== -->
                                            ...
                                            @if(count($products)>0)
                                                ...
                                                <tr id="item{{$product->id}}">
                                                    ...
                                                    <td>{{(($products->perPage() * $products->currentPage() - $products->perPage()) + ($index+1) )}}</td>
                                                     ...
                                                     <td><img src="{{asset('upload/images/product/thumb/'.$product->products->feature_image)}}" alt="Image" width="50"> <a target="_blank" href="{{ route('product_details', $product->products->slug) }}"> {{Str::limit($product->products->title, 40)}}</a></td>
													 
													 <td>
												   {{$product->stock}}												   
												   </td>
													 <td>{{$product->products->min_price}} - {{$product->products->max_price}}</td>
                                                   <td>
												   {{$product->selling_price}}												   
                                                    ...
                                                    <div class="btn-group">
                                                        ...
                                                        <div class="dropdown-menu">
                                                           <a class="dropdown-item" onclick="edit_modal({{$product->id}})" title="Edit product" data-toggle="tooltip" href="javascript:void(0)"><i class="ti-pencil-alt"></i> Edit Offer</a>
        ...
        <div class="modal fade" id="edit_modal" role="dialog" style="display: none;">
            <div class="modal-dialog">
                <form action="{{route('vendor.enlist.update')}}"  method="post">
                      {{ csrf_field() }}
                  <!-- Modal content-->
                  <div class="modal-content">
                    <div class="modal-header">
...
@section('js')
  ...
  function edit_modal(id){
            ...
            var  url = '{{route("vendor.enlist.edit", ":id")}}';
            url = url.replace(':id',id);
            $.ajax({
                url:url,
                method:"get",
