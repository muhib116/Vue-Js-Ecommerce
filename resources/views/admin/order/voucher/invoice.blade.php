        ...
        <div class="container-fluid">
            ...
            <div class="row page-titles">
                ...
                <div class="col-md-7 align-self-center text-right">
                    <div class="d-flex justify-content-end align-items-center">
                        <a href="{{route('admin.voucherOrderList')}}" class="btn btn-info d-none d-lg-block m-l-15"><i class="fa fa-eye"></i> Back Voucher list</a>
                    </div>
                </div>
            </div>
            <!-- ============================================================== -->
            ...
            <div class="container">
                <div class="card card-body printableArea" style="position: relative;">
                    ...
                    <h3><b>INVOICE NO: </b> <span class="pull-right">#{{$order->invoice_id}}</span></h3>
                    <hr>
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="pull-right text-right">
                                <address>
                                <strong>Order ID:</strong> #{{$order->invoice_id}} <br>
                                ...
                                @if($order->shipping_method)<b>Shipping Method:</b> {{ $order->shipping_method }}@endif
                                </address>
                            </div>
                        </div>
                        @endif
                ...
                <div class="text-right no-print">
                    <input id="invoice_id" type="hidden" name="invoice_id" value="{{$order->invoice_id}}">
                    <input type="hidden" id="all_orders" name="all_orders" value="{{$order->order_id}}">
                    @if((Auth::guard('admin')->user()->role_id == 'admin' || $order->payment_status == 'paid') && ($order->order_status != 'closed' && $order->order_status != 'cancel'))
                    @if(Auth::guard('admin')->user()->role_id == 'admin' || in_array(Auth::guard('admin')->user()->id, [19,22,25]) )
                    @if(Auth::guard('admin')->user()->role_id == 'admin' || $order->invoicePrints <= 0 )
                    <button id="print" onclick="invoicePrintBy('{{$order->invoice_id}}')" class="btn btn-success btn-outline" type="button"> <span><i class="fa fa-print"></i> Print</span> </button>
                    @endif
                    @endif
                    @else
                    <span style="color: red;">Order Status: {{$order->order_status}}</span>
                    @endif
...
@section('js')
    ...
    function invoicePrintBy(order_id) {
            ...
            var link = '{{route("admin.voucherInvoicePrintBy", ":order_id")}}';
