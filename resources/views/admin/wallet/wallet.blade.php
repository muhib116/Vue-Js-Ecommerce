...
@section('content')
                                            ...
                                            @if(count($allWallets)>0)
                                                ...
                                                <tr>
                                                   ...
                                                   <td>{{ $wallet->type }}</td>
                                                    ...
                                                    <td>
                                                        @if(in_array(Auth::guard('admin')->user()->id, [1,7,30]) && $wallet->type == 'walletRecharge' && $wallet->status == 'pending')
                                                            <select style="padding:3px;" onchange ="walletRechargeStatus('{{$wallet->id}}', this.value)" class="form-control" id="status">
                                                                <option @if($wallet->status== 'pending') selected @endif value="pending">Pending</option>
                                                                <option @if($wallet->status== 'paid') selected @endif value="paid">Approve</option>
                                                                <option @if($wallet->status== 'cancel') selected @endif value="cancel">Cancel</option>
                                                            </select>
                                                        @else
                                                            @if($wallet->status == 'paid') <span class="label label-success"> {{$wallet->status}}</span> @elseif($wallet->status == 'received') <span class="label label-primary"> {{$wallet->status}} </span> @elseif($wallet->status == 'cancel') <span class="label label-danger"> {{$wallet->status}} </span> @else <span class="label label-info"> {{$wallet->status}} </span> @endif
                                                        @endif
                                                    </td>
                                                </tr>
                                               @endforeach
    ...
    <script type="text/javascript">
       ...
       $('#getCustomerDetails').on('click', function(){
        ...
        function walletRechargeStatus(id, status){
            if (confirm("Are you sure "+status+ " this wallet.?")) {
                $.ajax({
                    url:"{{route('customer.walletRechargeStatus')}}",
                    method:"post",
