<?php  
$products = App\Models\Product::where('status', 'active')->where('is_b2b', 0)
->selectRaw('products.id,title,selling_price,discount,discount_type, slug, feature_image, is_b2b, profit, pricing')->whereRaw('id IN (select MAX(id) FROM products GROUP BY category_id)')->orderBy('id', 'desc')->take($section->item_number)->get(); 
?>
@if(count($products)>0)
   ...
   <div class="row">
      <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="background:{{$section->background_color}};display: block;margin: 0;padding: 10px;display: flex;justify-content: space-between;align-items: center;">
        <h4 style="color:{{$section->text_color}};margin: 0;">{{$section->title}}</h4>
										<div class="col -df -j-end -fsh0"> <a href="{{route('todayDeals')}}"  class="-df -i-ctr -upp -m -mls -pvxs">See All <i class="fa fa-chevron-right" aria-hidden="true"></i></a> </div>
