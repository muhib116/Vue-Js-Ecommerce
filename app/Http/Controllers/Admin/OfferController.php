...
class OfferController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['offerTypes'] = OfferType::orderBy('position', 'asc')->where('status', 1)->get();
        $data['offers'] = Offer::withCount(['offer_products','offer_orders'])->where('offer_type', '!=', 'quiz')->orderBy('position', 'asc')->get();
    ...
    public function store(Request $request)
    {
        ...
        $offer->prefix_id = strtoupper($request->prefix_id);
        ...
        $offer->notes = ($request->details) ? $request->details :null;
    ...
    public function updateOffer(Request $request){
        ...
        if($store) {
            ...
            if($offerProduct) {
                //update all offer products
                $data = [];
                if ($request->discount > 0) {
                    $data['offer_price'] = $request->discount;
                    $data['offer_discount'] = $request->discount;
                }
                //update discount type
                $data['discount_type'] = $request->discount_type;
                OfferProduct::where('offer_id', $request->id)->update($data);
            }
            Toastr::success('Offer update successful.');
        }else{
    ...
    public function offerSingleProductStore(Request $request)
    {
        ...
        if($offer){
            ...
            if(!$offerProduct) {
				...
				$offerProduct->campaign = $offer->offer_type;
                $offerProduct->fake_sale = rand(00,99);
                $offerProduct->offer_price = $request->offer_discount;
                $offerProduct->offer_quantity = ($request->quantity) ? $request->quantity : 0;
                $offerProduct->timeend = $request->timeend;
				$offerProduct->timestart = $request->timestart;
				$offerProduct->viewlimit = $request->viewlimit;
				$offerProduct->viewlimits = $request->viewlimit;
				$offerProduct->percentend = $request->percentend;
				$offerProduct->percentstart = $request->percentstart;
			...
			//	$ruuning = Offer::where('start_date', '<=', Carbon::now())->where('end_date', '>=', Carbon::now())->pluck('id')->toArray();
			
					
					
					
					$offerp = OfferProduct::where('offer_id', $offer->id)->where('product_id', $request->product_id)->first();
					...
					if($offerp != null){
						$output = [
                    'status' => false,
                    'msg' => 'Product were featured on a campaign previously.'
                ];
					} else {
        ...
        if($offer && $request->product_id){
            foreach ($request->product_id as $product_id => $value) {
                ...
                if(!$offerProduct){
                    ...
                    $offerProduct->offer_price = $request->discount[$product_id];
                    $offerProduct->offer_quantity = ($request->quantity[$product_id]) ? $request->quantity[$product_id] : 0;
                    $offerProduct->timeend = $request->timeend[$product_id];
				$offerProduct->timestart = $request->timestart[$product_id];
				$offerProduct->viewlimit = $request->viewlimit[$product_id];
				$offerProduct->viewlimits = $request->viewlimit[$product_id];
				$offerProduct->percentend = $request->percentend[$product_id];
				$offerProduct->percentstart = $request->percentstart[$product_id];
                    $offerProduct->approved = 1;
                    $offerProduct->status = 'active';
					...
					$offerp = OfferProduct::where('offer_id', $offer->id)->where('product_id', $product_id)->first();
					
					
					if($offerp != null){
						Toastr::error('Product was already featured on campaign previously.');
    ...
    public function offerProductUpdate(Request $request)
    {
        ...
        if($offerProduct) {
			...
			if($offerProduct->campaign == 'jowar-bhata'){
				$offerProduct->viewlimit = $request->viewlimit;
				$offerProduct->viewlimits = $request->viewlimit;
    ...
    public function getAllProducts (Request $request){
        ...
        if($data['offer']) {
			...
			$pdata = OfferProduct::where('offer_id', $request->offer_id)->pluck('product_id')->toArray();
            ...
            $products = Product::where('status', '!=', 'pending')->whereNotIn('id', $pdata);
            if ($request->product) {
    ...
    public function offerOrder(Request $request, $offer_slug, $status=''){
        ...
        if($data['offer']){
            ...
            $offer_orders = Order::join('users', 'orders.user_id', 'users.id')
            ...
            if($status){
                $offer_orders->where('order_status', $status)->orderBy('order_date', 'asc');
            }else{
                $offer_orders->orderBy('order_date', 'desc');
            }
	...
	public function delivery(){
		$delivery = Day::all();
		return view('admin.offer.delivery', compact('delivery'));
	}
	
	public function delivery_edit($id){
		$delivery = Day::find($id);
		return view('admin.offer.delivery_edit', compact('delivery'));
	}
	 ...
	 public function delivery_store(Request $request)
    {
		...
		$store = $day->save();
        if($store) {
            Toastr::success('Delivery Configuration created successfully.');
        }else{
            Toastr::error('Delivery Configuration can\'t created.');
        }
        return back();
	...
	public function delivery_update(Request $request)
    {
		...
		$store = $day->save();
        if($store) {
            Toastr::success('Delivery Configuration updated successfully.');
        }else{
            Toastr::error('Delivery Configuration can\'t updated.');
        }
        return back();
	 ...
	 public function delivery_delete($id)
    {
        ...
        if($day){
            ...
            $output = [
                'status' => false,
                'msg' => 'Configuration cannot deleted.'
            ];
        }
        return response()->json($output);
    }
