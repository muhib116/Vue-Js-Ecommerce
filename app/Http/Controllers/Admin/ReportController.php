...
class ReportController extends Controller
{
    ...
    {
$key = Keyword::orderBy('search', 'desc')->select('id','text','search','product')->get();

return view('admin.reports.search')->with(compact('key'));
    ...
    public function orderReport(Request $request)
    {
        ...
        if ($request->customer) {
            ...
            $orders->where(function ($query) use ($keyword) {
                ...
                $query->orWhereHas('order_details.product', function($query)  use ($keyword) {
