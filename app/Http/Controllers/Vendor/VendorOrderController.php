...
class VendorOrderController extends Controller
{
    ...
    public function orderHistory(Request $request, $status='')
    {
		...
		$orderCount = OrderDetail::where('is_b2b', 0)->whereHas('order', function($q){
    $q->where('payment_method', '!=', 'pending');
})->where('vendor_id', $vendor_id)->selectRaw('shipping_status, id, count(*) as aggregate')->groupBy('order_id')->get();
	   ...
	   $orders = OrderDetail::where('is_b2b', 0)->whereHas('order', function($q){
    $q->where('payment_method', '!=', 'pending');
})->where('vendor_id', $vendor_id)->orderBy('id', 'desc')->groupBy('order_id');
        ...
        $orders = $orders->selectRaw('order_details.*, count(qty) as quantity, sum(qty*price) as total_price')->paginate(15);
	  ...
	  public function b2borderHistory(Request $request, $status='')
    {
		...
		$orderCount = OrderDetail::where('is_b2b', 1)->whereHas('order', function($q){
    $q->where('payment_method', '!=', 'pending');
})->where('vendor_id', $vendor_id)->selectRaw('shipping_status, id, count(*) as aggregate')->groupBy('order_id')->get();
	   ...
	   $orders = OrderDetail::where('is_b2b', 1)->whereHas('order', function($q){
    $q->where('payment_method', '!=', 'pending');
})->where('vendor_id', $vendor_id)->orderBy('id', 'desc')->groupBy('order_id');
        ...
        $orders = $orders->selectRaw('order_details.*, count(qty) as quantity, sum(qty*price) as total_price')->paginate(15);
        $cancelReasons = OrderCancelReason::where('order_id', null)->where('status', 1)->get();
        return view('vendors.order.b2borders')->with(compact('orders', 'orderCount', 'cancelReasons'));
    }
    ...
    public function b2bshowOrderDetails($orderId){
        $vendor_id = Auth::guard('vendor')->id();
        $order = Order::where('is_b2b', 1)->with(['seller_order_details.product:id,title,slug,feature_image','get_country', 'get_state', 'get_city', 'get_area'])
            ->where('order_id', $orderId)->first();
        if($order){
            return view('vendors.order.b2border-details')->with(compact('order'));
        }
        return false;
    }
    ...
    public function showOrderDetails($orderId){
        $vendor_id = Auth::guard('vendor')->id();
        $order = Order::where('is_b2b', 0)->with(['seller_order_details.product:id,title,slug,feature_image','get_country', 'get_state', 'get_city', 'get_area'])
            ->where('order_id', $orderId)->first();
        if($order){
    ...
    public function changeOrderStatus(Request $request){
        $vendor_id = Auth::guard('vendor')->id();
        $order = Order::where('is_b2b', 0)->with('order_details')->where('order_id', $request->order_id)->first();
        if($order){
