...
class FrontPagesController extends Controller
{
    ...
    public function page($slug)
    {
        ...
        if($data['page']){
            ...
            if($slug == 'pre-order'){
                $data['products'] = Product::where('product_type', 'pre-order')->orderBy('id', 'desc')->where('status', 'active')->paginate(24);
            }if($slug == 'live-session'){
                $data['liveSessions'] = LiveSession::with(['liveProducts.product:id,feature_image'])->orderBy('position', 'asc')->where('status', 1)->paginate(24);
            }
    ...
    public function todayDeals()
    {
        ...
        if($data['page']){
            ...
            $data['banners'] = Banner::where('page_name', 'all')->orWhere('page_name', $data['page']->id)->orderBy('position', 'asc')->where('status', 1)->get();
    ...
    public function b2b()
    {
        ...
        if($data['page']){
            $data['products'] = Product::orderBy('id', 'desc')->where('status', 'active')->where('is_b2b', 1)->paginate(24);
            $data['banners'] = Banner::where('page_name', 'all')->orWhere('page_name', $data['page']->id)->orderBy('position', 'asc')->where('status', 1)->get();
            return view('frontend.pages.b2b')->with($data);
        }
        return view('404');
    }
    ...
    {
        $data['page'] = Page::where('slug', \Request::segment(1))->where('status', 1)->first();
$recent = Visitor::where('ip', visitorip())->whereNotNull('product')->first();
        if($data['page']){
            if($recent != null){
            $data['products'] = Product::where('status', 'active')->whereNotNull('keyword')->where(function ($query) use($recent) {
          foreach(json_decode($recent->product, true) as $pick){
           ...
           $data['products'] = Product::orderBy('id', 'desc')->where('status', 'active')->paginate(24); 
            ...
            $data['banners'] = Banner::where('page_name', 'all')->orWhere('page_name', $data['page']->id)->orderBy('position', 'asc')->where('status', 1)->get();
            return view('frontend.pages.recommand')->with($data);
        }
        return view('404');
    }
    ...
    public function megaDiscount ()
    {
        ...
        if($data['page']){
            $data['products'] = Product::where('discount', '!=', null)->orderBy('id', 'desc')->where('status', 'active')->paginate(24);
            $data['banners'] = Banner::where('page_name', 'all')->orWhere('page_name', $data['page']->id)->orderBy('position', 'asc')->where('status', 1)->get();
    ...
    {
        $data['brands'] = Brand::withCount('products')->where('brands.status', 1)->orderBy('name')->get();
...
$keyproduct = $data['products']->count();
			
			
			
			
			if(!empty($keyproduct) && !empty($data['brand']->keyword)){
			...
			if($service != null){
				...
				$olddata = explode(',', $data['brand']->keyword);		

				if(!empty($service->brand)){
				$catkey->brand = json_encode(array_unique(array_merge(json_decode($catkey->brand, true),$olddata), SORT_REGULAR));
			} else {
				$catkey->brand = json_encode(explode(',', $data['brand']->keyword));
			}
