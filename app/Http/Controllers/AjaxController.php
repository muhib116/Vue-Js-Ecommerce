...
class AjaxController extends Controller
{
    ...
    public function get_city($id=0){
        ...
        if(count($cities)>0){
            ...
            foreach($cities as $city){
                $output .='<option '. ((Auth::check() && Auth::user()->city == $city->id) || Session::get('city') == $city->id ? "selected" : "" ).' value="'.$city->id.'">'.$city->name.'</option>';
            }
        }
        echo $output;
    }
    ...
    public function deleteVariation($id)
    {
        ...
        if($delete){
            $variationDetails = ProductVariationDetails::where('variation_id', $id);
    ...
    public function getCartHead()
    {
        ...
        if(count($getCart)>0){
            ...
            echo '<h4 style="color:#ff0000;text-align: center;padding: 0px">Your cart is empty.</h4>';
        }
    }
    ...
    public function satusActiveDeactive(Request $request){
        ...
        if(is_numeric($status->$field)){
            ...
            $value =  ($status->$field == 'active' ? ($request->table == 'products' || $request->table == 'affiliate_products' ? 'deactive' : 'pending') : 'active');
        }
        if($status){
    ...
    public function setAnyItemValue(Request $request){
        ...
        if(request()->isMethod('get')) {
            $output = array(
                'status' => 'success',
                'msg' => str_replace('_', ' ',$request->field) .' update successful.'
            );
            return response()->json($output);
        }
