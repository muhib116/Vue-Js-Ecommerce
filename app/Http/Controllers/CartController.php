    ...
    public function cartAdd(Request $request)
    {
        ...
        $getOffer = Offer::join('offer_products', 'offers.id', 'offer_products.offer_id')->join('products', 'offer_products.product_id', 'products.id')
			...
			if ($carttaka >= 1) {
				...
				if($request->product_id == 2759 || $request->product_id == 2758){
				$output = array(
                'status' => 'error',
                'msg' => 'Sorry, You Already Exceeds Offer Buying Limit'
            );
            return $output;
				}
			...
			$carttaka = Cart::where('product_id', 2759)->where('offer_id', 84)->where('user_id', $user_id)->count();
			if ($carttaka >= 3) {
				
				if($request->product_id == 2760 || $request->product_id == 2758){
				$output = array(
                'status' => 'error',
                'msg' => 'Sorry, You Already Exceeds Offer Buying Limit'
            );
			...
			$carttaka = Cart::where('product_id', 2758)->where('offer_id', 84)->where('user_id', $user_id)->count();
			if ($carttaka >= 3) {
				
				if($request->product_id == 2760 || $request->product_id == 2759){
				$output = array(
                'status' => 'error',
                'msg' => 'Sorry, You Already Exceeds Offer Buying Limit'
            );
            ...
            $discount_type = $getOffer->discount_type;
			 ...
			 if (in_array($offer_id, ['83', '80']) && $qty > 5) {


$output = array(
                'status' => 'error',
                'msg' => 'Maximum Limit For This Item Is 5'
            );
            return $output;
}
...
if (in_array($cart->offer_id, ['83', '80']) && $request->qty > 5) {

return response()->json(['status' => 'error', 'msg' => 'Maximum Limit For This Item Is 5']);
...
$carttaka = Cart::where('product_id', 2760)->where('offer_id', 84)->where('user_id', $user_id)->count();
	...
	$fivetaka = OrderDetail::where('product_id', 2760)->where('offer_id', 84)->where('user_id', $user_id)->count();
			if ($fivetaka >= 1) {
				...
				if($cart->product_id == 2759 || $cart->product_id == 2758){
				return response()->json(['status' => 'error', 'msg' => 'Sorry, You Already Exceeds The Buying Limits For The Item']);
			...
			if ($fivetaka >= 3) {
				...
				if($cart->product_id == 2760 || $cart->product_id == 2758){
				return response()->json(['status' => 'error', 'msg' => 'Sorry, You Already Exceeds The Buying Limits For The Item']);
			...
			if ($fivetaka >= 3) {
				...
				if($cart->product_id == 2760 || $cart->product_id == 2759){
				return response()->json(['status' => 'error', 'msg' => 'Sorry, You Already Exceeds The Buying Limits For The Item']);
...
if (in_array($cart->offer_id, ['83', '80'])){
...
if(!in_array($cartname->offer_id, ['80', '83'])) {
    ...
    public function buyDirect(Request $request)
    {
        ...
        if($getOffer){
			...
			$onetaka = Cart::where('product_id', 2760)->where('offer_id', 84)->where('user_id', $user_id)->count();
			if ($onetaka >= 1) {
				
				if($product->id == 2759 || $product->id == 2758){
			...
			$onetaka = Cart::where('product_id', 2759)->where('offer_id', 84)->where('user_id', $user_id)->count();
			if ($onetaka >= 3) {
				
				if($product->id == 2760 || $product->id == 2758){
				Toastr::error('Sorry, You Already Exceeds The Buying Limits For The Item');
            return redirect()->back();
				}
			...
			$onetaka = Cart::where('product_id', 2758)->where('offer_id', 84)->where('user_id', $user_id)->count();
			if ($onetaka >= 3) {
				
				if($product->id == 2760 || $product->id == 2759){
				Toastr::error('Sorry, You Already Exceeds The Buying Limits For The Item');
            return redirect()->back();
				}
