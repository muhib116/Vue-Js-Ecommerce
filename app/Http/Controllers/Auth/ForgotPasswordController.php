    ...
    */
    ...
    public function passwordRecoverNotify(Request $request){
        ...
        if($fieldType == 'email'){
			...
			$otpcount = Verification::where('phone', $user->mobile)->where('date', $date)->first();
			...
			$verification->phone = $user->mobile;
            ...
            if ($user) {
			 ...
			 return redirect()->back()->withErrors(['emailOrMobile' => trans('Password Reset With Phone Daily Limit Exceeds .Please Try Again Tommorow')]);
			}
        }
