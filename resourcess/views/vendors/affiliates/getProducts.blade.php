    ...
    @foreach($allproducts as $index => $product) 
        ...
        <tr @if($product->affiliate_product || $offer_price < $affiliate_configure->minimum_offer_price) style="background: #ffe2e2" @else id="product{{  $product->id }}" @endif>
            ...
            <td>
                ...
                <input type="number" style="min-width:70px;" class="form-control" onkeyup ="commissionCalculate({{$product->id}}, {{$offer_price}}, this.value )" value="{{round(($product->affiliate_product) ? $product->affiliate_product->seller_rate : $offer_price)}}" id="seller_rate{{ $product->id }}" name="seller_rate[{{ $product->id }}]">
            
            <i style="font-size:11px;color: #616060;">Price less than  {{Config::get('siteSetting.currency_symble') . round($offer_price) }}</i>

            </td>
            <td><input type="number" min="{{ $affiliate_configure->time_duration }}" class="form-control" value="{{ ($product->affiliate_product) ? $product->affiliate_product->day : $affiliate_configure->time_duration }}" id="day{{ $product->id }}" placeholder="Enter day"  name="day[{{ $product->id }}]"> <i style="font-size:11px;color: #616060;">Minimum day {{$affiliate_configure->time_duration}} </i></td>
