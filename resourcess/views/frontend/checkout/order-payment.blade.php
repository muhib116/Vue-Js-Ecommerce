...
@section('content')
	...
	<div class="container">
        ...
        @php $grand_total = $order['total_price'] + $order['shipping_cost'] - $order['coupon_discount']; @endphp
		  ...
		  @php
		  $prepay = (Config::get('siteSetting.cod') / 100) * $grand_total;
                   ...
                   <div id="process"></div>  
                      ...
                      <ul class="nav nav-tabs">
                          @foreach($paymentgateways as $index => $method)
                            ...
                            <!-- //check offer for wallet balance -->
                            @if(($order->offer_id == null || $order->order_type == 'quiz')  || $method->method_slug != 'wallet-balance')
                          ...
                          <div id="paymentgateway{{$method->id}}" class="tab-pane fade @if($index == 0) active in @endif">
                              <form action="{{route('order.payment', [$order['order_id']])}}" method="post" @if($method->method_slug == 'masterCard') class="require-validation" data-cc-on-file="false" data-stripe-publishable-key="{{$method->public_key}}"  @endif >
                                  ...
                                  @endif
								   ...
								   @if($method->method_slug == 'cash-on-delivery')
									   
								   
								  @if(Config::get('siteSetting.cod') > 0)
                                    <h3></h3><h3 style="text-align: center;"><span style="color: rgb(206, 0, 0);"><span style="font-size: 1.5rem;"> You Need To Pay : <font color="#ff00ff"><b>{{ $order['currency_sign'] .  $prepay }}</font> For Place This COD Order</span><span style="font-size: 1.5rem; color: rgb(255, 0, 255);"><br></span><span style="font-size: 1.5rem;">Bkash&nbsp;Merchant</span></span>   <font color="#ff00ff"><b>0131 6100 326</b></font></h3>
                              ...
                              <p><input type="text" required data-parsley-required-message = "Transaction Id is required" placeholder="Enter Transaction Id" value="{{old('trnx_id')}}" class="form-control" name="trnx_id"></p>
								...
								<strong>Sending Bkash No</strong>
                              <p><input type="text" required data-parsley-required-message = "Sending Account No" placeholder="Sending Account No" value="{{old('account')}}" class="form-control" name="account"></p>
