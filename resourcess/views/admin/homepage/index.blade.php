                                    ...
                                    <tbody id="positionSorting" data-table="homepage_sections">
                                        ...
                                        <tr style="background:{{$data->background_color}};color: {{$data->text_color}}" id="item{{$data->id}}">
                                            ...
                                            <td>
                                                ...
                                                @if($data->section_type == 'categories')
                                                <a title="Add item under this section" class="btn btn-primary btn-sm" href="{{route('admin.categorySection')}}"><i class="ti-pin-alt" aria-hidden="true"></i> Manage {{$data->section_type}}</a>
                                                @endif
...
@section('js')
    ...
    function sectionType(sectionType, edit=''){
        ...
        if(sectionType== 'banner'){
            ...
            output = '<div class="col-md-12"><div class="form-group"> <label class="required" for="product_id">Product Categories</label> <select name="product_id" required="required" id="product_id" class="form-control select2" custom-select"><option value="">Select Category</option> @foreach($categories as $category)  <option value="{{$category->id}}">{{$category->name}}</option> <!-- get subcategory --> @if(count($category->get_subcategory)>0) @foreach($category->get_subcategory as $subcategory)  <option value="{{$subcategory->id}}">&nbsp; -{{$subcategory->name}}</option> <!-- end subcategory --> @endforeach  @endif <!-- end subcategory --> @endforeach</select> </div></div>';
